...
export default function EvalBar({ evaluation, boardOrientation = 'white' }) {
  ...
  const topLabel = isFlipped ? 'W' : 'B';
  const bottomLabel = isFlipped ? 'B' : 'W';

  return (
    <div className="flex flex-col items-center gap-1.5 select-none">
      <span className="text-[10px] font-mono text-text-dim font-bold">{topLabel}</span>
      <div className="relative w-7 h-full min-h-[320px] rounded-md bg-white overflow-hidden border border-border-glass shadow-inner">
        {/* Black fill */}
        <motion.div
          className={`absolute left-0 right-0 bg-[#1e1e1e] ${isFlipped ? 'bottom-0' : 'top-0'}`}
          animate={{ height: `${100 - whitePercent}%` }}
          transition={{ duration: 0.4, ease: 'easeOut' }}
        />

        {/* Eval text indicator */}
        <div className={`relative z-10 w-full h-full flex flex-col justify-between p-1.5 pointer-events-none ${isFlipped ? 'flex-col-reverse' : ''}`}>
      ...
      <span className="text-[10px] font-mono text-text-dim font-bold">{bottomLabel}</span>
    </div>
  );
}
